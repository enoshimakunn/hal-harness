...
class Agent(ABC):
    ...
    async def _find_final_answer(self, response_text: str) -> str:
        ...
        if final_answer_pattern.search(response_text):
            # Try to extract answer with sources format first
            final_answer_match = re.search(
                r"FINAL ANSWER:\s*(.*?)(?:\{['\"]sources['\"]|$)",
                response_text,
                re.DOTALL | re.IGNORECASE,
            )
            
            if not final_answer_match or not final_answer_match.group(1).strip():
                # Fallback: get everything after "FINAL ANSWER:"
                final_answer_match = re.search(
                    r"FINAL ANSWER:\s*(.*)",
                    response_text,
                    re.DOTALL | re.IGNORECASE,
            ...
            agent_logger.info(f"\033[1;32m[FINAL ANSWER]\033[0m {final_answer[:200]}...")
            return final_answer
    ...
    def _shorten_message_history(self):
        ...
        # Remove messages from position 1 until we have removed enough
        # Keep removing until we've removed at least 2 items or hit bottom
        removed_count = 0
        while len(self.messages) > 2 and removed_count < 4:
            self.messages.pop(1)
            removed_count += 1

        agent_logger.info(f"Removed {removed_count} message(s) from history")
    ...
    async def run(self, question: str, session_id: str = None) -> tuple[str, dict]:
        ...
        self.messages: list = [initial_message]
